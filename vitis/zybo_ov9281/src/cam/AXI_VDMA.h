...
#define AXI_VDMA_CONFIG_WRITE 0
#define AXI_VDMA_CONFIG_READ  1
...
public:
	...
	u32 init(){
		...
		xil_printf("INFO [AXI_VDMA::init()] Initializing VDMA Config\r\n");
		status = XAxiVdma_CfgInitialize(&drv_inst_, psConf, psConf->BaseAddress);
		xil_printf("INFO [AXI_VDMA::init()] VDMA Config initialized\r\n");
		return status;
	}
	...
	XStatus configureRead(uint16_t h_res, uint16_t v_res) {
		XStatus status;

		initMemAxiVdma(h_res, v_res, AXI_VDMA_CONFIG_READ);

        // Set again h/v attr
        context_.ReadCfg.HoriSizeInput = (h_res * drv_inst_.ReadChannel.StreamWidth) >> 3; // div by 8
		...
		status = XAxiVdma_ClearDmaChannelErrors(&drv_inst_, XAXIVDMA_WRITE, XAXIVDMA_SR_ERR_ALL_MASK);
		if(status != XST_SUCCESS){
			xil_printf("ERROR [AXI_VDMA::configureRead] Failed to clear write channel errors\r\n");
			return status;
		}
        ...
        if (XST_SUCCESS != status){
			xil_printf("ERROR [AXI_VDMA::configureRead] Failed to configure write channel\r\n");
			return status;
		}
		...
		if (XST_SUCCESS != status){
			xil_printf("ERROR [AXI_VDMA::configureWrite] Failed to set write channel buffer address\r\n");
			return status;
		}
	...
	XStatus enableRead(){
		return XAxiVdma_DmaStart(&drv_inst_, XAXIVDMA_READ);
	}
	...
	XStatus configureWrite(uint16_t h_res, uint16_t v_res) {
        ...
        initMemAxiVdma(h_res, v_res, AXI_VDMA_CONFIG_WRITE);
        ...
        status = XAxiVdma_ClearDmaChannelErrors(&drv_inst_, XAXIVDMA_WRITE, XAXIVDMA_SR_ERR_ALL_MASK);
		if(status != XST_SUCCESS){
			xil_printf("ERROR [AXI_VDMA::configureWrite] Failed to clear write channel errors\r\n");
			return status;
		}
		...
		if (XST_SUCCESS != status){
			xil_printf("ERROR [AXI_VDMA::configureWrite] Failed to configure write channel\r\n");
			return status;
		}
    ...
    void initMemAxiVdma(uint16_t hRes, uint16_t vRes, u32 read_write) {
		int iFrm = 0;
		h_res_pix = hRes;
		v_res_pix = vRes;
		...
		for (iFrm = 0; iFrm < drv_inst_.MaxNumFrames; iFrm++){
			...
			if(read_write == AXI_VDMA_CONFIG_READ)
				context_.ReadCfg.FrameStoreStartAddr[iFrm] = tmp_addr;

			if(read_write == AXI_VDMA_CONFIG_WRITE)
