#!/usr/bin/env python3
"""
blob_detect_rle_verify.py — End-to-end RLE blob detection verification with visualization.
...
Usage:
  ...
  python helper_scripts/blob_detect_rle_verify.py [--width 1280] [--height 800] [--num-blobs 8] [--seed 42]
"""
...
IP_DIR    = os.path.join(PROJ_ROOT, "vivado", "ip_repo", "blob_detect_rle")
...
def run_functional_model(hex_path, width, height, threshold, work_dir):
...
def run_xsim(width, height, threshold, work_dir):
    sim_out = os.path.join(work_dir, "sim_out", "tb_blob_detect_rle")
    ...
    srcs = [
        os.path.join(rdl_dir, "blob_detect_rle_regs_pkg.sv"),
        os.path.join(rdl_dir, "blob_detect_rle_regs.sv"),
        os.path.join(hdl_dir, "stream_fifo.sv"),
        os.path.join(hdl_dir, "run_extractor.sv"),
        os.path.join(hdl_dir, "row_merger.sv"),
        os.path.join(hdl_dir, "blob_table.sv"),
        os.path.join(hdl_dir, "blob_detect_rle_top.sv"),
        os.path.join(hdl_dir, "blob_detect_rle_wrapper.v"),
        os.path.join(SIM_DIR, "tb_blob_detect_rle.sv"),
    ]
    ...
    vivado_cmd("xelab tb_blob_detect_rle -snapshot tb_sim -debug all --log xelab.log")
...
def main():
    parser = argparse.ArgumentParser(description="RLE blob detection end-to-end verify + visualize")
    ...
    if args.work_dir:
        ...
        work_dir = tempfile.mkdtemp(prefix="blob_rle_verify_")
    ...
    model_blobs, model_json = run_functional_model(
    ...
    plt.tight_layout()
    out_png = os.path.join(work_dir, "blob_rle_verify.png")
