...
- Driver exposes `MEDIA_BUS_FMT_Y10_1X10` and `MEDIA_BUS_FMT_Y8_1X8` (mono) — **both mono codes are incompatible with the Xilinx CSI-RX for 8/10-bit** (see the STREAMON EPIPE gotcha below). The fix is to make the sensor advertise a Bayer8 code instead.
...
- **`VIDIOC_STREAMON` returns `-EPIPE` (Broken pipe) — sensor↔CSI mbus mismatch.** The OV9281 is a mono sensor; mainline `ov9282.c` advertises only `Y8_1X8`/`Y10_1X10`. **The Xilinx CSI2RxSS driver has no mono code for 8/10-bit** — its format table (`xilinx-csi2rxss.c`, ~line 182) maps RAW8 → `SRGGB8/SBGGR8/SGBRG8/SGRBG8` and RAW10 → the four Bayer10 codes only; mono appears only at RAW12 (`Y12_1X12`). So the CSI sink pad **cannot hold `Y8_1X8`** — its `set_fmt` silently coerces any Y8 request to `SGRBG8_1X8` (visible in `media-ctl -p`).
  - **Where it fails:** `STREAMON` → `xilinx-dma`'s `video_device_pipeline_start()` walks every enabled link calling `v4l2_subdev_link_validate`, which requires code+W+H to match across each link. It fails on the **sensor→CSI link** (`ov9281 src = Y8_1X8` ≠ `CSI sink = SGRBG8_1X8`) and returns `-EPIPE`. This is a software negotiation check at stream-start — no pixels have moved. 6.6 returns EPIPE **without** a dmesg line (enable `/sys/kernel/debug/dynamic_debug/control` for diagnostics).
  - **Not VDMA, not an ov9282 bug.** VDMA's own check (`xvip_dma_verify_format`: DMA pixelformat ↔ CSI *source* pad) is downstream and already satisfied (`GRBG`↔`SGRBG8`). Mainline `ov9282` is correct (OV9281 really is mono); it normally pairs with format-agnostic CSI bridges. The Xilinx CSI is the Bayer-centric outlier.
  - **On the wire `Y8` and any `Bayer8` are identical** — both CSI-2 data type `0x2A`, same bytes. The Bayer label is cosmetic for a mono sensor.
  - **Fix (chosen): one-line CSI LUT addition.** Add `{ MIPI_CSI2_DT_RAW8, MEDIA_BUS_FMT_Y8_1X8 }` to `xcsi2dt_mbus_lut[]` in `xilinx-csi2rxss.c` (~line 186). `xcsi2rxss_set_format` accepts any code whose `get_dt()` is RAW8 ("RAW8 is supported in all datatypes", csi2rxss.c:811), so with Y8 in the LUT the sink pad takes it without coercion and the source pad mirrors it → `Y8` end to end. **No sensor patch:** mainline `ov9282.c` already emits `Y8_1X8` natively for 8-bit. **No VIP patch:** `xilinx-vip.c:97` already maps `Y8_1X8 ↔ V4L2_PIX_FMT_GREY`, so the DMA node exposes honest grayscale (`GREY`), not a misleading Bayer fourcc. Tracked as `recipes-kernel/linux/linux-xlnx/0001-xilinx-csi2rxss-add-Y8_1X8-mono-RAW8-mbus-code.patch` (registered in the `.bbappend` `SRC_URI`). Run the pipeline as `Y8_1X8`/`GREY` (e.g. `mocap-sanity --format GREY`), **not** `SGRBG8`/`GRBG`.
  - **Why not patch `ov9282.c` instead** (the rejected alternative): it'd advertise `SGRBG8_1X8` (3 hunks) and work, but mislabels mono data as Bayer — any format-aware consumer (GStreamer, OpenCV `cvtColor`) would then debayer and corrupt the global-shutter mono image. The CSI route keeps the sensor pristine and the labeling honest.
- **`mocap-sanity` app** (`recipes-apps/mocap-sanity/`) is a self-contained scripted capture test: walks `/dev/media0` topology (`MEDIA_IOC_G_TOPOLOGY`), resolves sensor + CSI subdev devnodes by entity-name substring, sets all pad formats (`VIDIOC_SUBDEV_S_FMT`, sensor src pad 0 / CSI sink 0 / CSI src 1), then `S_FMT`+MMAP-captures one frame to a file. Defaults `/dev/video0` → `./image_capture.raw`, `GRBG` 1280×800; `--mbus-code` and `--skip-setup` overrides. Subdev-coercion warnings surface the EPIPE mismatch above before STREAMON. Entity name is `ov9281 6-0060` (i2c-client name from compatible, **not** the `ov9282` driver filename).
